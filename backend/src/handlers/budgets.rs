...
pub async fn create_budget(
    ...
    let result = sqlx::query(
        "INSERT INTO budgets (location_id, name, amount, start_date, end_date)
         VALUES (?, ?, ?, ?, ?)"
    )
...
pub async fn list_budgets(
    ...
    let budgets = if let Some(location_id) = params.location_id {
        sqlx::query_as("SELECT * FROM budgets WHERE location_id = ? ORDER BY start_date DESC")
            .bind(location_id)
            .fetch_all(pool.as_ref())
            .await
    } else {
        sqlx::query_as("SELECT * FROM budgets ORDER BY start_date DESC")
            .fetch_all(pool.as_ref())
            .await
    }
    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn get_budget(
    ...
    let budget = sqlx::query_as("SELECT * FROM budgets WHERE id = ?")
        .bind(id)
        .fetch_one(pool.as_ref())
        .await
...
pub async fn update_budget(
    ...
    sqlx::query(
        "UPDATE budgets SET location_id = ?, name = ?, amount = ?,
