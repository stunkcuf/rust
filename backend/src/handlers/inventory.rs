...
pub async fn list_inventory(
...
) -> Result<Json<Vec<InventoryWithDetails>>, (StatusCode, String)> {
    let mut query = "SELECT
        i.id, i.item_id, it.name as item_name, it.sku,
        i.location_id, l.name as location_name, i.quantity,
        it.reorder_level, it.overstock_level, it.unit_price, i.last_updated
        FROM inventory i
    ...
    if params.location_id.is_some() {
        query.push_str(" AND i.location_id = ?");
    }
    if params.item_id.is_some() {
        query.push_str(" AND i.item_id = ?");
    }
...
pub async fn get_inventory(
    State(pool): State<Arc<SqlitePool>>,
    Path(id): Path<i64>,
) -> Result<Json<InventoryWithDetails>, (StatusCode, String)> {
    ...
    .await
    .map_err(|_| (StatusCode::NOT_FOUND, "Inventory record not found".to_string()))?;
...
pub async fn update_inventory(
    ...
    let existing: Option<Inventory> = sqlx::query_as(
    ...
    if let Some(inv) = existing {
        ...
        let updated: Inventory = sqlx::query_as(
            "SELECT * FROM inventory WHERE item_id = ? AND location_id = ?"
        )
        .bind(payload.item_id)
        .bind(payload.location_id)
        ...
        let inventory: Inventory = sqlx::query_as("SELECT * FROM inventory WHERE id = ?")
...
pub async fn get_low_stock(
    ...
    let inventory = sqlx::query_as(
        ...
        JOIN items it ON i.item_id = it.id
        JOIN locations l ON i.location_id = l.id
        WHERE i.quantity <= it.reorder_level
...
pub async fn get_overstock(
    ...
    let inventory = sqlx::query_as(
        ...
        JOIN items it ON i.item_id = it.id
        JOIN locations l ON i.location_id = l.id
        WHERE i.quantity >= it.overstock_level
