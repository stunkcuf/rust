...
pub async fn create_purchase(
    ...
    let result = sqlx::query(
        "INSERT INTO purchases (item_id, location_id, budget_id, quantity, unit_price, total_price, vendor, ordered_by)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    )
    ...
    // Update budget spent if budget_id is provided
...
pub async fn list_purchases(
    State(pool): State<Arc<SqlitePool>>,
    Query(params): Query<PurchaseQuery>,
) -> Result<Json<Vec<PurchaseWithDetails>>, (StatusCode, String)> {
    let mut query = "SELECT
        p.id, p.item_id, i.name as item_name, i.sku,
        p.location_id, l.name as location_name,
        p.budget_id, b.name as budget_name,
        p.quantity, p.unit_price, p.total_price, p.vendor, p.status,
        p.ordered_by, u.username as ordered_by_username, p.ordered_at
        FROM purchases p
        JOIN items i ON p.item_id = i.id
        JOIN locations l ON p.location_id = l.id
        LEFT JOIN budgets b ON p.budget_id = b.id
        JOIN users u ON p.ordered_by = u.id
        WHERE 1=1".to_string();

    if params.location_id.is_some() {
        query.push_str(" AND p.location_id = ?");
    }
...
pub async fn get_purchase(
    State(pool): State<Arc<SqlitePool>>,
    Path(id): Path<i64>,
) -> Result<Json<PurchaseWithDetails>, (StatusCode, String)> {
...
pub async fn update_purchase_status(
    State(pool): State<Arc<SqlitePool>>,
    Path(id): Path<i64>,
    Json(payload): Json<UpdatePurchaseStatus>,
) -> Result<Json<Purchase>, (StatusCode, String)> {
    ...
    if payload.status == "completed" {
        ...
        if let Some((current_qty,)) = existing {
            ...
            .bind(current_qty + purchase.quantity)
            .bind(purchase.item_id)
            .bind(purchase.location_id)
            .execute(pool.as_ref())
            .await
    ...
    let purchase = sqlx::query_as("SELECT * FROM purchases WHERE id = ?")
        .bind(id)
        .fetch_one(pool.as_ref())
        .await
        .map_err(|_| (StatusCode::NOT_FOUND, "Purchase not found".to_string()))?;
